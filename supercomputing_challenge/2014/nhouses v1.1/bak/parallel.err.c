		...
		for (j=column-1; j>=0; j--) {
	   	...
	   	if (is_sol) {
		  	if (column == GRANULARITY-1) {
				// If this is the level (granularity of the job),
...
int worker_build_house(int nx, int column, int site[]) {
   	int num_sol = 0;
	bool is_sol;
	int i, j;
	   	...
	   	if (is_sol) {
		  	if (column == nx-1) {
				...
				num_sol += 1;
			   	...
			   	// The placement is not complete.
				// Try to place the house on the next column
				num_sol += worker_build_house(nx, column+1, site);
		   	}
	   	}
   	}
...
int send_job_worker(int site[], int nx) {
