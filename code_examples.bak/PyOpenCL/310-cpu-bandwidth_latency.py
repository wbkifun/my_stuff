...
for i, nx in enumerate(seed_nxs):
	...
	clib.memcpy_htoh.argtypes = [c_int, arg, arg]

	t0 = datetime.now()
	for j in xrange(memcpy_iterations):
		clib.memcpy_htoh(nx, in_host, out_host)
	dt = datetime.now() - t0
...
f['cpu'][cpu_name].create_dataset('nx_copy', data=seed_nxs)
f['cpu'][cpu_name].create_dataset('dt_copy', data=elapsed_times)
f.close()
