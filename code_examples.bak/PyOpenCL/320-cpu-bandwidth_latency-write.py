...
for i, nx in enumerate(seed_nxs):
	...
	clib.memwrite.argtypes = [c_int, arg]

	t0 = datetime.now()
	for j in xrange(memcpy_iterations):
		clib.memwrite(nx, in_host)
	dt = datetime.now() - t0
...
f['cpu'][cpu_name].create_dataset('nx_write', data=seed_nxs)
f['cpu'][cpu_name].create_dataset('dt_write', data=elapsed_times)
f.close()
