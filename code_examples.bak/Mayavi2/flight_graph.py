...
for line in routes_data.split('\n'):
    if re.match(r'[\s\w]+,[\s\w]+', line):
        #routes_table.append( re.split(r',[\w]*', line) )
        routes_table.append( [x.strip() for x in line.split(',')] )

# Build a dictionnary returning GPS coordinates for each city
cities_coord = dict()
for line in cities_data.split('\n'):
    if re.match(r'[\s\w]+,[-\s\w.]+,[-\s\w.]+', line):
        #name, lon, lat = re.split(r',[\w]*', line)
        name, lon, lat = [x.strip() for x in line.split(',')]
        cities_coord[name] = (float(lon), float(lat))
...
# positioning.
lat, lon = coords.T * np.pi/180
x = np.cos(lon) * np.cos(lat)
y = np.cos(lon) * np.sin(lat)
z = np.sin(lon)
