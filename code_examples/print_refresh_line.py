...
def print_progress(t0, step, max_step):
    ...
    if step == 1:
        print('[{:>{ms}}] [progress] [elapsed time] [estimated end time]'.format(
            'step', ms=step_width))

    print('[{:>4d}] [{:>6.1f} %] [{:>12s}] [{:>18s}]'.format(\
            step, 
            (step)/max_step*100, 
            str(timedelta( seconds=int(dt.seconds) )), 
            str(timedelta( seconds=int(max_step*dt.seconds/step) )) ), # estimated end time
            flush=True, end='\r')
