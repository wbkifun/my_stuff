...
def check_sparse_matrix_on_mvp(f, dsts, srcs, weights, mvps):
    ...
    '''
    #-----------------------------------------------------
    # Avg the element boundary for the spectral element method
...
def test_se_random():
    '''
    SparseMatrixAvg: Same values on the MVP with random numbers (ne=5,ngq=4)
    '''
    ne, ngq = 5, 4
...
def test_se_sequential():
    '''
    SparseMatrixAvg: Same values on the MVP with sequential numbers (ne=5,ngq=4)
    '''
    ne, ngq = 5, 4
    spmat = SparseMatrixAvg(ne, ngq)
