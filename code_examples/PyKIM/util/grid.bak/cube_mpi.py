...
class CubeMPI:
    ...
    def make_mpi_tables(self):
        ...
        recv_buf_size = local_buf_size \
                + sum([d_unique.size for d_unique in recv_group.values()])
...
if __name__ == '__main__':
    ...
    ngq = 4
    ne = args.ne
    #dpath = "/scratch/khkim/mpi_tables_ne{}_nproc{}".format(ne,nproc)    # GAON2
    dpath = "./mpi_tables_ne{}_nproc{}".format(ne,nproc)

    if myrank == 0:
        print("Generate the MPI tables for Implicit diffusion")
        print("ne={}, ngq={}, target_nproc={}".format(ne,ngq,nproc))
